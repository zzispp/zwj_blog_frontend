...
import { getNotesApi, type GetNotesRequest } from "@/lib/note-api";
import { type GetNotesDTO } from "../types";

// 将前端参数转换为后端参数格式
function convertToBackendParams(params: GetNotesDTO): GetNotesRequest {
  // 处理published字段的映射
...
function convertToFrontendResponse(backendResponse: any) {
  return {
    notes: backendResponse.data.items.map((item: any) => ({
      id: item.id.toString(),
      title: item.title,
      slug: item.slug,
...
export const useGetNotes = (params: GetNotesDTO) => {
  return useRequest(
    async () => {
      ...
      const response = await getNotesApi(backendParams);
